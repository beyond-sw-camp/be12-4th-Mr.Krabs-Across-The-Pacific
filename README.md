...
4. [📝 CI/CD 환경 설정](#-cicd-환경-설정)
5. [🎞️ 프론트엔드 CI/CD 시나리오](#%EF%B8%8F-프론트엔드-cicd-시나리오)
6. [🎞️ 백엔드 CI/CD 시나리오](#%EF%B8%8F-백엔드-cicd-시나리오)
7. [▶️ 배포 영상](#%EF%B8%8F-배포-영상)
...
![Architecutre](./images/CICDArchitecture.svg)
...
2. Jenkins로 Webhook이 전달되어 Jenkinsrk CircleCI API를 호출, CircleCI에서 프로젝트를 빌드 및 Docker 이미지 푸시 트리거
   - 전달 값:
...
* 모든 노드는 사전에 SSH 키를 교환했다고 가정한다.
* 모든 노드에 적용되는 부분

  ![공통](/images/k8s%20common.gif)

* 마스터에서 실행해야 하는 부분: 컴퓨팅 자원 문제로 2개 노드(마스터 + 1개 워커)로 인벤토리를 바꿔 실행
  * 쿠버네티스 대시보드는 포트를 직접 사람이 봐야하므로 자동화에서 제외

  ![마스터에서 실행](/images/k8s%20init%20and%20join.gif)

</details>

### 백엔드 CI/CD 동작 및 배포 결과

![](./images/backend_pipeline.png)     
Jnekins, CircleCI의 파이프라인

![git push](./images/1.git_push.gif)      
1. git push를 하면 GitHub에서 Jenkins로 WebHook을 보낸다.

![CircleAPI](./images/2.CircleAPI.gif)      
2. Jenkins에서 Pipeline의 가장 첫 단계인 Trigger CirclrCI API가 작동해 CircleCI가 실행된다.

![CircleCI_pipeine](./images/3.CircleCI_pipeline.gif)      
3. CircleCI 내부 파이프라인에서 gradle-build와 docker build&push가 순차적으로 동작한다.      
Jenkisn는 CircleCI가 success/fail 여부를 보낼 때까지 기다린다.

![succes](./images/4.success_deploy.gif)      
4. CircleCI가 파이프라인 작업을 완료하면 Jenkins가 성공여부를 확인한다.      
성공시 빌드 버전에 맞춰 blue 혹은 green을 선택해 K8s에 배포를 명령, 실패시 작업을 중단한다.
