...
1. git push 후 GitHub에서 Jenkins로 WebHook을 보냄

![CircleAPI](./images/2.CircleAPI.gif)
2. Jenkins에서 Pipeline의 가장 첫 단계인 Trigger CirclrCI API가 작동해 CircleCI 실행

Jenkisn는 CircleCI가 success/fail 여부를 보낼 때까지 기다림

![CircleCI_pipeine](./images/3.CircleCI_pipeline.gif)
3. CircleCI 내부 파이프라인에서 gradle-build와 docker build&push가 순차적으로 동작

![succes](./images/4.success_deploy.gif)
4. CircleCI가 생성한 Docker Image를 DockerHub에 push 성공시키면 Jenkins가 이를 확인 후 K8S에 배포 명령

![deploy](./images/5.deploy_complete.gif)
5.K8S가 push된 Docker Image를 이용하여 Container배포